...
const EditPost = () => {
  ...
  const updatePostMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["post", id] }); // Refresh cache
      navigate(`/posts/${id}`);
    },
    onError: (error) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.image) {
      formDataToSend.append("image", formData.image); // Append only if a new image is selected
    }
    formDataToSend.append("published_at", formData.published_at);
  
    await updatePostMutation.mutateAsync(formDataToSend);
  };
