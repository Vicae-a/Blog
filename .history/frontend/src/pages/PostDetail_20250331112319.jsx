...
const PostDetail = () => {
  ...
  let imageUrl = post?.image_path 
    ? (post.image_path.startsWith('http') ? post.image_path : `http://localhost:8000/storage/${post.image_path}`)
      ...
      <div className="container">
        <article className="post">
          ...
          <div className="post-image">
            <img src={imageUrl} alt={post?.title || "Post Image"} onError={(e) => e.target.src = 'https://via.placeholder.com/800x400'} />
          </div>
        ...
        <section className="comments-section">
          ...
          {isAuthenticated ? (
            <CommentForm onSubmit={(commentData) => addCommentMutation.mutate(commentData)} postId={post?.id} />
          ) : (
          ...
          <div className="comments-list">
            {comments.length === 0 ? (
              ...
              comments.map(comment => (
                <Comment key={comment.id} comment={comment} onDelete={() => deleteCommentMutation.mutate(comment.id)} />
              ))
            )}
          </div>
        </section>
