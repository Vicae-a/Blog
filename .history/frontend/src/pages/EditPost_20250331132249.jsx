      ...
      <div className="container">
        ...
        <form onSubmit={handleSubmit} className="post-form">
          <div className="form-group">
            ...
            <input type="text" id="title" name="title" value={formData.title} onChange={handleChange} maxLength={255} />
          ...
          <div className="form-group">
            ...
            {imagePreview && (
              <div className="image-preview">
                ...
                <button type="button" className="remove-image" onClick={() => { setImagePreview(null); setFormData({ ...formData, image: null }); }}>Remove Image</button>
              </div>
            )}
          </div>
          <div className="form-group">
            <label htmlFor="published_at">Publish Date (Optional)</label>
            <input type="datetime-local" id="published_at" name="published_at" value={formData.published_at} onChange={handleChange} />
            {errors.published_at && <span className="error">{errors.published_at}</span>}
          </div>
          <div className="form-buttons">
            <button type="button" className="btn btn-secondary" onClick={() => navigate(`/posts/${id}`)}>Cancel</button>
            <button type="submit" className="btn btn-primary" disabled={updatePostMutation.isLoading}>{updatePostMutation.isLoading ? 'Saving...' : 'Update Post'}</button>
          </div>
        </form>
      </div>
    </div>
