...
const PostDetail = () => {
  ...
  console.log("Post Data:", post); // Debugging to verify structure

  // Check multiple possible fields for author's name
  const postAuthor = post.user?.name || post.user?.username || post.author?.name || "Unknown Author";

  // Ensure image URL is properly formed
  let imageUrl = 'https://via.placeholder.com/800x400'; // Default placeholder
  if (post.image_path) {
      ...
      <div className="container">
        <article className="post">
          ...
          <div className="post-actions">
            <button onClick={() => navigate(`/posts/${post.id}`)} className="read-more">Back to Post</button>
            {isAuthor && (
              <>
